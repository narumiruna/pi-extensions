...
- [x] Add a patch Changeset for the final `@narumitw/pi-todo` notice release and update its README migration section with the duplicate-load warning, availability check, persistent-scope remove/install order, temporary/local source-switching guidance, restart instruction, and an explicit deferral of unimplemented replacement compatibility claims until Stage 2 is published and verified. Evidence: `.changeset/announce-pi-progress-migration.md`; after the baseline 0.3.3 release consumed `fast-ts-runtime-graphs.md`, `npm exec changeset status --verbose` reports the remaining `@narumitw/pi-todo` migration-notice patch intent.
...
- [x] Publish the final predecessor version through the normal Changesets version PR and `publish.yml`, then verify npm serves the new `pi-todo` README, description, changelog/version, and functional tarball. Evidence: release PR #1358 published `@narumitw/pi-todo@0.3.4`; npm serves the migration README and description; the nine-file tarball registers only `update_todo_list`; all predecessor versions remain available.

### Stage 2: implement and publish the replacement

- [x] Record the replacement contract in focused tests before changing implementation: assert the sole registered tool is `update_progress`; its strict public schema is `{ steps: [{ text, status, reason? }] }`; only blocked steps accept `reason`; at most one step is `in_progress`; and empty `steps` clears state. Evidence: the focused test first failed with `update_todo_list` received instead of `update_progress`; the final schema and validation suite passes.
- [x] Rename `packages/pi-todo/` to `packages/pi-progress/`, update the package manifest to `@narumitw/pi-progress`, repository directory, description, keywords, source/test filenames, build-owned temporary prefixes, root `pi.extensions` entry, and workspace lockfile through root `npm install`. Evidence: `npm ls @narumitw/pi-progress` resolves `0.3.4` from `packages/pi-progress`; active manifests and root entries contain no predecessor package; npm serves `pi-todo@0.3.4`; boundaries pass.
- [x] Rename the authoritative runtime vocabulary from Todo to Progress and adopt `ProgressStep.text`, `ProgressDetails.steps`, `update_progress`, label `Progress`, widget key `progress`, and Progress result/header/summary text without changing status behavior or limits. Evidence: focused tool, renderer, width, hostile-text, completion-summary, non-TUI, and lifecycle tests pass; canonical result assertions exclude `todos` and `step`.
- [x] Introduce version 4 canonical details and hidden context serialization for `{ steps: [{ text, status, reason? }] }`, with new Progress-owned custom message and restored-boundary identifiers. Evidence: focused tests assert version 4 writes, canonical hidden context, restart, clear, branch replacement, tree navigation, and Progress boundary metadata.
- [x] Add strict historical decoding for `update_todo_list` versions 1, 2, and 3 and `todo_widget` versions 1 and 2, including valid tool-call arguments, successful result details, clears, blocked reasons, limits, and the single-active-step invariant. Evidence: table-driven tests cover every supported row, exact matching call arguments, clears, wrong names and versions, errors, extra fields, malformed and oversized text, blocked reasons, multiple active steps, and stale snapshots.
- [x] Preserve old Todo hidden context and restored-boundary entries during their existing leading-summary epoch while deduplicating both old and new extension-owned custom messages; emit only canonical Progress context for a new summary epoch. Evidence: reload, clear, idempotence, duplicate-message, later-epoch, and sibling-branch tests preserve old bytes only for the owning epoch and branch.
- [x] Update the normalized cache-contract suite for the intentional tool-definition and system-guidance transition: establish the first `update_progress` request as the new prefix baseline, then verify ordered active tools, provider-visible definition, effective guidance, and serialized message prefixes remain stable across ordinary turns, updates, clears, reloads, and compaction restoration. Evidence: normalized exact-prefix tests pass for the transition, ordinary requests, updates, clears, reloads, old boundaries, and later canonical epochs.
- [x] Rename settings types and defaults to Progress terminology, make `pi-progress.json` canonical, and add read-only `pi-todo.json` fallback only when the canonical path is absent. Evidence: settings tests cover canonical and legacy precedence, invalid canonical without fallback, invalid and unsafe legacy files, missing files, malformed and oversized input, UTF-8, cancellation, stale async completion, and byte-for-byte no-write behavior.
- [x] Update `README.md` under `packages/pi-progress` to the required structure and Progress terminology, including `update_progress`, the exact `steps[].text` example, canonical and legacy settings behavior, package build/load commands, old-package removal then new-package installation, session compatibility, duplicate-install warning, security/privacy, limitations, and package layout. Evidence: implementation review matches the documented interfaces and a fenced-code-aware audit passes for all 35 package READMEs.
- [x] Preserve predecessor history in the renamed changelog and add a Changeset describing the new package/tool/schema contract and historical-session compatibility. Evidence: `.changeset/launch-pi-progress.md` records a minor replacement release and `npm exec changeset status --verbose` reports only `@narumitw/pi-progress`.
- [x] Audit the final diff against `docs/extension-conventions.md`, `docs/extension-settings.md`, and `docs/readme-conventions.md`: review cancellation and disposal, session replacement and shutdown, every post-`await` ownership check, state-decoder equivalence classes, prompt-prefix epochs, widget key cleanup, settings precedence/no-write behavior, package boundaries, and all user-visible names. Evidence: lifecycle and decoder suites cover the applicable equivalence classes; settings and cache audits pass; the README documents the intentional read-only filename migration deviation.
- [x] Run the focused Progress tests, `npm --workspace @narumitw/pi-progress run build --if-present`, `npm run check`, and plain `npm test` sequentially. Evidence: 52 focused tests pass; the package build and typecheck pass; `npm run check` passes; all 5,101 repository tests pass.
- [x] Run `npm run package:pack -- progress`, inspect the tarball file list and manifest, then smoke `pi --no-extensions -e ./packages/pi-progress` after a clean package build. Evidence: the ten-file tarball has the expected manifest and source/runtime files; the Jiti loader test registers only `update_progress`, updates and clears `progress`, and shuts down cleanly; the package-directory smoke lists 462 models.
...
- [x] A final functional `pi-todo` release notifies TUI/RPC users, documents the exact gated migration command, preserves the old tool/context contract, and is available from npm before the repository rename.
- [x] The repository contains `packages/pi-progress` and no active `packages/pi-todo` workspace or root Pi entry after the predecessor release completes.
- [x] The replacement registers only `update_progress` with the exact canonical `steps[].text` schema and writes only version 4 Progress details.
- [x] Valid historical `update_todo_list` and `todo_widget` branches restore correctly; malformed or unrelated history never changes state.
- [x] Old restored context remains byte-stable for its summary epoch, new epochs use canonical Progress context, and normalized ordinary-request prefixes remain stable after the intentional rename transition.
- [x] The widget, timers, settings load, tree navigation, session replacement, non-TUI modes, and shutdown retain existing behavior under Progress-owned names.
- [x] `pi-progress.json` wins when present, `pi-todo.json` is a read-only fallback only when canonical settings are absent, and neither file is modified.
- [x] Both READMEs prevent duplicate installation and accurately describe availability checks, package/tool identities, payload, settings, privacy, limitations, historical-session support, and rollback commands.
...
- [x] Focused tests, package builds, `npm run check`, `npm test`, package pack inspections, and Pi Jiti load smokes pass with evidence in the handoff.
