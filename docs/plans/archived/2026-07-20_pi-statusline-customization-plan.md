...
- Normalize a canonical JSON document into field-level defaults for named palettes, density, separator, ordered segments, per-segment `prefix`/`suffix`, and extension icons.
...
- [x] Add failing settings tests for full/partial normalization, invalid fields, unknown preservation, environment independence, first-run creation, races, migration, and atomic failures; implemented the expanded settings model and persistence in `src/settings.ts`, verified by the focused 45-test pi-statusline run.
...
- [x] Run package formatting/typecheck, root tests/check, `just pack-statusline`, and a headless offline Pi RPC smoke; package check and `npm run check` passed with 835 tests, the 12-file tarball excludes tests/classic, and the smoke generated a 1,216-byte default config without publishing.
