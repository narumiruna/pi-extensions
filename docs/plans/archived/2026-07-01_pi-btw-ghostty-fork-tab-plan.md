...
- [x] Add fallback behavior: if no session file exists or AppleScript fails/times out, notify a warning and use the existing inline `/btw` flow; verified by no-session and AppleScript-failure tests in `extensions/pi-btw/test/btw.test.ts`.
