...
- [x] **Stage 1 — Enforce mode invariants.** Added direct `tool_call` blocking for `update_plan` while preserving built-in mutation enforcement; hook tests pass in the 301-test root suite.
...
- [x] **Stage 6 — Add optional Plan thinking level.** Added validated `pi-plan-mode.json` inherit/fixed settings with ownership-aware restoration; tests cover entry, resume, provider clamping, manual changes, implementation handoff, normal exit, and session shutdown.
...
- [x] Repository quality gates pass with `npm run check` (301/301 tests), and extension loading is smoke-tested non-interactively with `pi -ne -e ./extensions/pi-plan-mode --help` showing `--plan`.
