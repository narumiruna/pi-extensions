...
test("sanitizeTraceValue globally bounds adversarial values in UTF-8 bytes", () => {
  ...
  const value = {
    manyKeys: Object.fromEntries(Array.from({ length: 1_000 }, (_, index) => [`key-${index}`, "🪢".repeat(100)])),
  ...
  assert.deepEqual(
    sanitizeTraceValue({
      ...
      embeddedDataUri: "example: data:application/octet-stream;base64,not-valid%%% should not be parsed",
    }),
    {
...
test("sanitizeTraceValue bounds string work before redaction and UTF-8 sizing", () => {
  ...
  Buffer.byteLength = ((value: unknown, encoding?: BufferEncoding) => {
    if (typeof value === "string") {
      assert.ok(value.length <= MAX_CAPTURE_BYTES, "sanitizer scanned the complete oversized string");
    }
  ...
  RegExp.prototype[Symbol.replace] = function (this: RegExp, value: string, replacement: unknown) {
    assert.ok(value.length <= MAX_CAPTURE_BYTES, "sanitizer redacted the complete oversized string");
    return Reflect.apply(originalReplace, this, [value, replacement]) as string;
  } as (typeof RegExp.prototype)[typeof Symbol.replace];

  try {
    const sanitized = sanitizeTraceValue(`data:text/plain;base64,${"a".repeat(MAX_CAPTURE_BYTES * 4)}`);
...
test("sanitizeTraceValue stops enumerating object properties at the collection cap", () => {
  const value = Object.fromEntries(Array.from({ length: 1_000 }, (_, index) => [`key-${index}`, `value-${index}`]));
