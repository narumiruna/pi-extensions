...
test("TraceRecorder bounds oversized tool details as one captured output", () => {
  ...
  recorder.finishTool("call", {
    ...
    details: Object.fromEntries(Array.from({ length: 500 }, (_, index) => [`detail-${index}`, "value".repeat(500)])),
  });
...
test("TraceRecorder exports recovered HTTP diagnostics and native generation detail", () => {
  ...
  recorder.finishAttempt({ role: "assistant", content: "done", stopReason: "stop" });
  recorder.settle();
  assert.equal(backend.observations[0]?.updates.at(-1)?.metadata?.["pi.trace.outcome"], "recovered_success");
  assert.equal(backend.observations[0]?.updates.at(-1)?.metadata?.["pi.trace.recovered_error_count"], 1);
});
...
test("TraceRecorder distinguishes final error, aborted, length, and interrupted roots", () => {
  ...
  assert.equal(backend.observations[0]?.updates.at(-1)?.metadata?.["pi.trace.outcome"], "interrupted");
...
test("TraceRecorder closes duplicate, parallel, no-progress, and abrupt tools once", () => {
  ...
  assert.equal(tools[2]?.updates.at(-1)?.metadata?.["pi.tool.time_to_first_progress_ms"], undefined);
...
test("TraceRecorder stamps the session and user id on every observation", () => {
  ...
  assert.equal(backend.observations.find(({ name }) => name === "pi.agent")?.traceUpdates[0]?.userId, "user-42");
});
