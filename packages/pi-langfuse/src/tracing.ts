...
export class TraceRecorder {
  ...
  beginAgent(input: BeginAgentInput): void {
    ...
    const gitTag = input.git?.branch ? `branch:${input.git.branch}` : input.git?.detached ? "git:detached" : undefined;
  ...
  beginGeneration(input: GenerationInput = {}): void {
    ...
    const observation = this.startObservation(
      "pi.llm",
      {
        ...
        ...(input.thinkingLevel ? { modelParameters: { thinking_level: input.thinkingLevel } } : {}),
  ...
  finishAssistant(message: AssistantMessage): void {
    ...
    state.observation.update({
      ...
      ...(state.completionStartTime !== undefined ? { completionStartTime: new Date(state.completionStartTime) } : {}),
      ...
      metadata: {
        ...
        ...httpMetadata,
        ...(message.provider ? { "pi.provider": message.provider, "pi.response.provider": message.provider } : {}),
        ...
        ...(finiteNumber(message.usage?.reasoning) ? { "pi.usage.reasoning_tokens": message.usage.reasoning } : {}),
    ...
    tool.observation.update({
      ...
      metadata: {
        ...
        ...(tool.firstProgressAt !== undefined
          ? {
              "pi.tool.time_to_first_progress_ms": Math.max(0, tool.firstProgressAt - tool.startedAt),
            }
          : {}),
      },
      ...(result.isError ? { level: "ERROR" as const, statusMessage: "Pi tool execution failed." } : {}),
    });
      ...
      observation: this.startObservation(
        "pi.compaction",
        {
          metadata: {
            ...
            ...(finiteNumber(input.tokensBefore) ? { "pi.compaction.tokens_before": input.tokensBefore } : {}),
  ...
  private closeActiveTrace(statusMessage?: string, snapshot?: ContextSnapshot, forcedOutcome?: Outcome): void {
    ...
    if (!this.root) return;
    const baseOutcome = forcedOutcome ?? this.lastAttemptOutcome ?? classifyOutcome(this.lastAssistant);
    const recoveredErrorCount =
      this.counters.recoveredErrors + (baseOutcome === "success" ? this.counters.failedAttempts : 0);
    const traceOutcome = baseOutcome === "success" && recoveredErrorCount > 0 ? "recovered_success" : baseOutcome;
    const finalMetadata: Record<string, unknown> = {
      "pi.trace.schema_version": Number(TRACE_SCHEMA_VERSION),
      "pi.trace.outcome": traceOutcome,
      ...(this.lastAssistant?.stopReason ? { "pi.trace.stop_reason": this.lastAssistant.stopReason } : {}),
  ...
  private closeAttempt(message?: AssistantMessage, statusMessage?: string, forcedOutcome?: Outcome): void {
    if (!this.attempt) return;
...
function generationHttpMetadata(state: GenerationState): Record<string, unknown> {
  ...
  return {
    ...(state.statuses.length > 0
      ...
      : {}),
    ...(Object.keys(state.headers).length > 0 ? { "http.response.headers": { ...state.headers } } : {}),
  };
}
...
function structuralCompactionDetails(details: unknown): Record<string, unknown> {
  ...
  return {
    ...(Array.isArray(record.readFiles) ? { "pi.compaction.read_file_count": record.readFiles.length } : {}),
...
function classifyOutcome(message?: { role?: string; stopReason?: string; errorMessage?: string }): Outcome {
...
function positiveNumericRecord(values: Record<string, number | undefined>): Record<string, number> {
  return Object.fromEntries(
    Object.entries(values).filter((entry): entry is [string, number] => finiteNumber(entry[1]) && entry[1] > 0),
  );
}
