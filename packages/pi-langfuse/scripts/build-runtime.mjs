...
export async function buildRuntime({ outputDirectory = distDirectory, validateOutput = validateGeneratedFiles } = {}) {
  ...
  const stagingDirectory = await mkdtemp(join(dirname(resolvedOutputDirectory), ".pi-langfuse-dist-"));
...
export async function validateGeneratedFiles(outputDirectory) {
  ...
  for (const declarationPath of declarationFiles) {
    ...
    for (const match of source.matchAll(/(?:from\s+|import\s*)["'](\.\.?\/[^"']+)\.js["']/gu)) {
      const targetPath = join(dirname(declarationPath), `${match[1]}.d.ts`).replaceAll("\\", "/");
      if (!files.includes(targetPath)) {
...
async function generateDeclarations(outputDirectory) {
  const typescriptEntry = join(dirname(require.resolve("typescript")), "tsc.js");
  await execFileAsync(process.execPath, [
    typescriptEntry,
    "-p",
    join(packageRoot, "tsconfig.types.json"),
    "--outDir",
    outputDirectory,
  ]);
}
