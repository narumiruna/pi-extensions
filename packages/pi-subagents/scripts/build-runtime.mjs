...
export async function buildRuntime({ outputDirectory = distDirectory, validateOutput = validateGeneratedFiles } = {}) {
  ...
  const stagingDirectory = await mkdtemp(join(dirname(resolvedOutputDirectory), ".pi-subagents-dist-"));
...
async function prepareGeneratedRuntime(outputDirectory) {
  for (const entry of GENERATED_TS_ENTRIES) {
    ...
    const rewritten = source.replace(`sourceMappingURL=${entry}.js.map`, `sourceMappingURL=${entry}.ts.map`);
    if (rewritten === source) {
