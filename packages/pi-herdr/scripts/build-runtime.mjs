...
const FORBIDDEN_EAGER_INPUTS = ["src/herdr-menu.ts"];
const FORBIDDEN_EAGER_EXTERNALS = ["@narumitw/pi-tui-kit"];
const EAGER_PRESENTATION_LEAVES = new Set([
  "@narumitw/pi-tui-kit/editor-status-widget",
  "@narumitw/pi-tui-kit/terminal-text",
]);
...
export async function buildRuntime({ outputDirectory = distDirectory, validateOutput = validateGeneratedFiles } = {}) {
  ...
  const stagingDirectory = await mkdtemp(join(dirname(resolvedOutputDirectory), ".pi-herdr-dist-"));
  ...
  for (const outputPath of eagerOutputs) {
    ...
    for (const imported of output.imports ?? []) {
      if (
        ...
        !EAGER_PRESENTATION_LEAVES.has(imported.path) &&
        FORBIDDEN_EAGER_EXTERNALS.some(
          (dependency) => imported.path === dependency || imported.path.startsWith(`${dependency}/`),
        )
      ) {
...
async function assertSafeOutputDirectory(outputDirectory) {
  ...
  const isTestOutput = firstSegment?.startsWith(".pi-herdr-build-test-");
  if (
    relativeOutput === "" ||
    relativeOutput === ".." ||
