...
export async function buildRuntime({ outputDirectory = distDirectory, validateOutput = validateGeneratedFiles } = {}) {
  ...
  const stagingDirectory = await mkdtemp(join(dirname(resolvedOutputDirectory), ".pi-typesafe-search-dist-"));
...
async function assertSafeOutputDirectory(outputDirectory) {
  ...
  const isTestOutput = firstSegment?.startsWith(".pi-typesafe-search-build-test-");
  if (
    relativeOutput === "" ||
    relativeOutput === ".." ||
