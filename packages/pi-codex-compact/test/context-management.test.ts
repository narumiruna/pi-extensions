...
test("a synthesized context contract stays at the durable conversation tail", async () => {
	...
	const synthetic = (await contextHandler(
	...
	assert.equal(
		await contextHandler({ type: "context", messages: durableMessages }, current.current.ctx),
		undefined,
	);
});
...
test.each(["codex-first", "generic-first"] as const)(
	...
	async (order) => {
		...
		const loader = new DefaultResourceLoader({
			...
			noContextFiles: true,
			extensionFactories:
				order === "codex-first" ? [codexFactory, genericFactory] : [genericFactory, codexFactory],
		});
		await loader.reload();
		...
		for (const name of GENERIC_CONTEXT_TOOL_NAMES) {
			assert.equal(
				resolved.find((tool) => tool.definition.name === name)?.definition.label,
				`Foreign ${name}`,
			);
		}
...
test("re-enabling after a post-deactivation compaction restores a tail activation", async () => {
	...
	const activation = current.mock.sentMessages.at(-1)?.message as
	...
	persistLastSentCustomMessage(current);
	const modelMessages = buildContextEntries(
		current.entries,
		current.entries.at(-1)?.id ?? null,
	).flatMap(sessionEntryToContextMessages);
	const contextHandler = current.mock.events.get("context")?.[0];
	assert.ok(contextHandler);
	const projected = (await contextHandler(
		{ type: "context", messages: modelMessages },
		current.current.ctx,
	)) as { messages: AgentMessage[] } | undefined;
	assert.deepEqual(
		projected?.messages
...
test.each(["context", "compaction"] as const)(
	...
	async (boundary) => {
		...
		assert.equal(
			(current.mock.sentMessages.at(-1)?.message as { customType?: string } | undefined)
				?.customType,
			CONTEXT_DEACTIVATION_MESSAGE_TYPE,
		);
		assert.deepEqual(current.mock.rawPi.getActiveTools(), ["read"]);
			...
			getAllTools().map((toolInfo) => {
				...
				return {
					...
					...(description === "different description"
						? { description: "Another extension owns this tool." }
						: {}),
					sourceInfo: {
						...tool.sourceInfo,
...
test("tree navigation reloads branch lineage and releases old-branch rollover state", async () => {
	...
	assert.ok(tree);
	await tree(
		{ type: "session_tree", oldLeafId: "old-state", newLeafId: "new-state" },
		current.current.ctx,
	);
	...
	assert.equal(
		(
			current.mock.sentMessages.at(-1)?.message as
				| { details?: { currentWindowId?: string } }
				| undefined
...
test.each(["signal", "assistant response"] as const)(
	...
	async (source) => {
		...
		const current = setup(
			true,
			undefined,
			source === "signal" ? { signal: runController.signal } : {},
		);
		await start(current);
		await current.mock.events.get("agent_start")?.[0]({ type: "agent_start" }, current.current.ctx);
		await tool(current, "codex_compact_start_new_context").execute(
...
test("a completed rollover is not reused by another compaction before settlement", async () => {
	...
	assert.notEqual(
		second.compaction.details.currentWindowId,
		second.compaction.details.previousWindowId,
	);
	await current.mock.events.get("agent_settled")?.[0](
		{ type: "agent_settled" },
		current.current.ctx,
