...
function requireEnabled(runtime: ContextToolRuntime): void {
	if (!runtime.isEnabled()) {
		throw new Error(
			"Experimental context management is disabled; enable it in pi-codex-compact settings first",
		);
	}
}
...
export function registerExperimentalContextTools(
	pi: ExtensionAPI,
		...
		parameters: Type.Object(
			{
				...
				id: Type.Optional(
					Type.String({ description: "Stable history item ID or note name", maxLength: 256 }),
				),
				query: Type.Optional(
					Type.String({ description: "Case-insensitive search text", maxLength: 512 }),
				),
				cursor: Type.Optional(
					Type.String({ description: "Cursor returned by an earlier result", maxLength: 12 }),
				),
			},
			{ additionalProperties: false },
		),
