...
export function parseExperimentalContextDetails(value: unknown): ExperimentalContextDetails | undefined {
  if (!isRecord(value)) return undefined;
  try {
...
export function activeExperimentalCompaction(entries: readonly SessionEntry[]):
  ...
  for (let index = entries.length - 1; index >= 0; index -= 1) {
    ...
    return details ? { entry: entry as CompactionEntry<ExperimentalContextDetails>, details } : undefined;
  }
  return undefined;
}
