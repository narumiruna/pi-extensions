...
const EXTENSION_ENTRY_PATH = realpathSync(
...
export function createExperimentalContextManager(
	...
	const isEnabled = () =>
		(isConfigured() || removeToolsAtSettlement) && toolsAvailable && inspectToolUnit().complete;

	const isRoutingExperimental = () =>
		isConfigured() || removeToolsAtSettlement || fallbackDeactivationPending;
	...
	const applySettings = (ctx: ExtensionContext) => {
		...
		if (
			...
			!hasContextContract(messages, activeLineage)
		) {
	...
	return {
		...
		beforeCompact(event, ctx) {
			...
			const activeLineage = ensureLineage(ctx);
			const request =
				pending?.status === "requested" || pending?.status === "compacting" ? pending : undefined;
		...
		onCompact(event, ctx) {
			if (
				!isOwned(ctx) ||
				!ctx.sessionManager.getBranch().some((entry) => entry.id === event.compactionEntry.id)
			) {
				return;
			}
