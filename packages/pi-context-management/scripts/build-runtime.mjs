...
export async function buildRuntime({ outputDirectory = distDirectory, validateOutput = validateGeneratedFiles } = {}) {
  ...
  const stagingDirectory = await mkdtemp(join(dirname(resolvedOutputDirectory), ".pi-context-management-dist-"));
...
async function assertSafeOutputDirectory(outputDirectory) {
  ...
  const isTestOutput = firstSegment?.startsWith(".pi-context-management-build-test-");
  if (
    relativeOutput === "" ||
    relativeOutput === ".." ||
