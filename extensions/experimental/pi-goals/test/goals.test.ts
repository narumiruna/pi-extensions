...
const STALE_GOAL_TOOL_REASON =
	"Blocked stale /goals tool call after the goal stopped or was interrupted.";

test("goal registers command, status tools, and lifecycle hooks", () => {
	const mock = createMockPi();
	goals(mock.pi);

	assert.ok(mock.commands.has("goals"));
	assert.equal(mock.commands.has("goal"), false);
	assert.equal(typeof mock.commands.get("goals")?.getArgumentCompletions, "function");
	assert.deepEqual(
		mock.tools.map((tool) => tool.name),
		["goals_complete", "goals_blocked"],
	);
	const completionParameters = mock.tools.find((tool) => tool.name === "goals_complete")
	...
	const blockerDefinition = mock.tools.find((tool) => tool.name === "goals_blocked");
...
test("session start warns that pi-goals is experimental", async () => {
	const mock = createMockPi();
	...
	assert.ok(
		context.notifications.some(
			({ message, level }) =>
				level === "warning" && /pi-goals.*experimental Pi extension/i.test(message),
		),
	);
});

test("completeGoalsArguments suggests /goals subcommands and token options", () => {
	assert.deepEqual(
		completeGoalsArguments("")?.map((item) => item.label),
		["pause", "resume", "clear", "edit", "status", "push", "unshift", "pop", "shift", "--tokens"],
	);
	assert.deepEqual(
		completeGoalsArguments("")?.map((item) => item.description),
		[
			...
			"Append a goal to the queue",
			"Prepend and prioritize a goal",
	...
	assert.deepEqual(
		completeGoalsArguments("pa")?.map((item) => item.value),
		["pause"],
	);
	assert.deepEqual(
		completeGoalsArguments("pause")?.map((item) => item.value),
		["pause"],
	);
	assert.deepEqual(
		completeGoalsArguments("--t")?.map((item) => item.value),
		["--tokens "],
	);
	assert.deepEqual(
		completeGoalsArguments("edit ")?.map((item) => item.value),
		["edit --tokens "],
	);
	assert.deepEqual(
		completeGoalsArguments("edit --t")?.map((item) => item.value),
		["edit --tokens "],
	);
	assert.equal(completeGoalsArguments("ship objective"), null);
	assert.equal(completeGoalsArguments("edit objective"), null);
});
...
test("parseCommand parses budgets, quoted objectives, and management commands", () => {
	...
	assert.equal(parseCommand("pause now"), "Usage: /goals pause");
});
...
test("goal token usage subtracts its baseline and clamps branch rewinds", async () => {
	...
	await tracked.mock.commands.get("goals")?.handler("", tracked.ctx);
...
test("active elapsed time excludes stopped waits and survives active edits", async (t) => {
	...
	now += 4_250;
	await timed.mock.commands.get("goals")?.handler("pause", timed.ctx);
	...
	now += 100_000;
	await timed.mock.commands.get("goals")?.handler("", timed.ctx);
	assert.equal(requireLastGoal(timed.mock).timeUsedSeconds, 4.25);
	assert.match(timed.notifications.at(-1)?.message ?? "", /Active elapsed: 4s/);

	await timed.mock.commands.get("goals")?.handler("resume", timed.ctx);
	assert.equal(requireLastGoal(timed.mock).activeStartedAt, now);
	now += 2_750;
	await timed.mock.commands.get("goals")?.handler("edit revised timed objective", timed.ctx);
...
test("goal completion settles the active clock before clearing state", async (t) => {
	...
	await requireGoalTool(completed.mock, "goals_complete").execute(
		"timed-completion",
...
test("legacy active-time state migrates without counting offline or reload time", async (t) => {
	...
	now += 2_000;
	await legacy.mock.commands.get("goals")?.handler("", legacy.ctx);
	...
	now += 2_000;
	await reloaded.mock.commands.get("goals")?.handler("", reloaded.ctx);
...
test("buildGoalSystemPrompt escapes objective XML and includes goal_id guard rules", () => {
	...
	assert.match(prompt, /Only call the goals_complete tool after/);
...
test("all goal prompt paths share the goal_id guard and hardened audit", async () => {
	...
	assert.match(continuationPrompt, /<!-- pi-goals-continuation:[^\s>]+ -->/);

	await started.mock.commands.get("goals")?.handler("pause", started.ctx);
	await started.mock.commands.get("goals")?.handler("resume", started.ctx);
	...
	assert.match(resumedPrompt, /explicitly resumed the paused \/goals/i);

	await started.mock.commands.get("goals")?.handler("edit verify edited objective", started.ctx);
...
test("goals_complete requires current goal_id before validating summary", async () => {
	const { mock, ctx } = await startGoalForTest();
	const tool = requireGoalTool(mock, "goals_complete");
...
test("goals_complete rejects contradictory summaries and accepts verified completion", async () => {
...
test("goals_complete rejects stale goal_id after replacement, pause/resume, and clear", async () => {
	const replaced = await startGoalForTest();
	const replacementTool = requireGoalTool(replaced.mock, "goals_complete");
	const originalGoal = requireLastGoal(replaced.mock);

	await replaced.mock.commands.get("goals")?.handler("ship replacement objective", replaced.ctx);
	...
	const resumeTool = requireGoalTool(resumed.mock, "goals_complete");
	const beforePauseGoal = requireLastGoal(resumed.mock);
	await resumed.mock.commands.get("goals")?.handler("pause", resumed.ctx);
	...
	await resumed.mock.commands.get("goals")?.handler("resume", resumed.ctx);
	...
	const clearTool = requireGoalTool(cleared.mock, "goals_complete");
	const beforeClearGoal = requireLastGoal(cleared.mock);
	await cleared.mock.commands.get("goals")?.handler("clear", cleared.ctx);
...
test("goals_blocked rejects calls without an active goal", async () => {
	const mock = createMockPi();
	goals(mock.pi);
	const context = createMockContext();
	mock.events.get("session_start")?.[0]?.({}, context.ctx);
	const blockerTool = requireGoalTool(mock, "goals_blocked");
...
test("goals_blocked requires a current active goal and strict blocker evidence", async () => {
	const blocked = await startGoalForTest();
	const blockerTool = requireGoalTool(blocked.mock, "goals_blocked");
	const completionTool = requireGoalTool(blocked.mock, "goals_complete");
	...
	assert.equal(blocked.statuses.get("goals"), "blocked");
...
test("session persistence restores stopped states with resumable command hints", async () => {
	for (const [status, statusline] of [
		...
		assert.equal(restored.statuses.get("goals"), statusline);

		await restored.mock.commands.get("goals")?.handler("", restored.ctx);
		assert.match(restored.notifications.at(-1)?.message ?? "", new RegExp(`Status: ${status}`));
		assert.match(restored.notifications.at(-1)?.message ?? "", /\/goals resume/);
	}
});
...
test("resume safely reactivates every resumable stopped status and rotates goal_id", async () => {
	for (const status of ["paused", "blocked", "usage_limited", "budget_limited"] as const) {
		...
		await restored.mock.commands.get("goals")?.handler("resume", restored.ctx);
		...
		assert.equal(restored.statuses.get("goals"), "active 5/10");
...
test("resume rejects active goals and exhausted budgets without rotating goal_id", async () => {
	...
	await active.mock.commands.get("goals")?.handler("resume", active.ctx);
	...
	for (const status of ["paused", "blocked", "usage_limited", "budget_limited"] as const) {
		...
		await exhausted.mock.commands.get("goals")?.handler("resume", exhausted.ctx);
...
test("failed resume delivery restores the stopped state and original goal_id", async () => {
	...
	assert.equal(restored.statuses.get("goals"), "blocked");
...
test("failed start delivery clears a new goal and restores a replaced stopped goal", async () => {
	...
	await freshMock.commands.get("goals")?.handler("new objective", freshContext.ctx);
	assert.equal(lastGoalStatus(freshMock), null);
	assert.equal(freshContext.statuses.get("goals"), undefined);
	...
	await replacement.mock.commands.get("goals")?.handler("pause", replacement.ctx);
	...
	await replacement.mock.commands.get("goals")?.handler("replacement objective", replacement.ctx);
...
test("failed active edit delivery restores and pauses the prior goal", async () => {
	...
	await edited.mock.commands.get("goals")?.handler("edit changed objective", edited.ctx);
...
test("pause remains active-only for new stopped statuses", async () => {
	for (const status of ["blocked", "usage_limited", "budget_limited"] as const) {
		const restored = restoreGoalForTest(status);
		await restored.mock.commands.get("goals")?.handler("pause", restored.ctx);
		assert.match(restored.notifications.at(-1)?.message ?? "", /only active goals can be paused/i);
		assert.equal(
			restored.statuses.get("goals"),
...
test("pause aborts the current turn, blocks stale tools, and persists paused state", async () => {
	...
	assert.match(staleContinuation, /pi-goals-continuation/);

	await paused.mock.commands.get("goals")?.handler("pause", paused.ctx);

	assert.equal(pauseAborts, 1);
	assert.equal(lastGoalStatus(paused.mock), "paused");
	assert.equal(paused.statuses.get("goals"), "paused");
	assert.deepEqual(
...
test("clear removes goal state without aborting or blocking stale tools", async () => {
	...
	assert.equal(lastGoalStatus(cleared.mock), null);
	assert.equal(cleared.statuses.get("goals"), undefined);
	assert.deepEqual(
	...
	const tool = requireGoalTool(cleared.mock, "goals_complete");
...
test("clear releases stale tool-call block from a paused goal", async () => {
	...
	await paused.mock.commands.get("goals")?.handler("clear", paused.ctx);

	assert.equal(lastGoalStatus(paused.mock), null);
	assert.equal(paused.statuses.get("goals"), undefined);
	assert.equal(
		paused.mock.events.get("tool_call")?.[0]?.(
...
test("state changes between agent_end and agent_settled cancel stale continuation intent", async () => {
	for (const action of ["pause", "clear", "replace", "complete"] as const) {
		...
		if (action === "pause" || action === "clear") {
			await changed.mock.commands.get("goals")?.handler(action, changed.ctx);
		} else if (action === "replace") {
			await changed.mock.commands.get("goals")?.handler("replacement objective", changed.ctx);
		} else {
			await requireGoalTool(changed.mock, "goals_complete").execute(
...
test("tool_execution_end enforces budget once and injects one bounded wrap-up", async () => {
	...
	assert.equal(budgeted.statuses.get("goals"), "budget 12/10");
	...
	assert.equal(wrapUpMessage.customType, "goals-budget-wrap-up");
	...
	assert.match(String(wrapUpMessage.content), /goals_complete.*evidence/i);
	...
	assert.equal(
		budgeted.mock.events.get("tool_call")?.[0]?.(
			{ toolName: "goals_complete", toolCallId: "complete-after-budget", input: {} },
			budgeted.ctx,
		),
		undefined,
	);

	const completion = await requireGoalTool(budgeted.mock, "goals_complete").execute(
...
test("rejected completion closes a budget wrap-up without another model call", async () => {
	...
	const rejected = await requireGoalTool(budgeted.mock, "goals_complete").execute(
	...
	const retry = await requireGoalTool(budgeted.mock, "goals_complete").execute(
	...
	const contextResult = budgeted.mock.events.get("context")?.[0]?.(
		{
			messages: [
				...
				{ role: "custom", customType: "goals-budget-wrap-up", content: "stale" },
			],
		},
		budgeted.ctx,
...
test("budget edits require an actual increase before reactivating and rotate stale ids", async () => {
	...
	await budgeted.mock.commands.get("goals")?.handler("edit unchanged budget", budgeted.ctx);
	...
	const staleCompletion = await requireGoalTool(budgeted.mock, "goals_complete").execute(
		"stale-budget-completion",
...
test("agent_end maps abort, quota failure, and terminal error to distinct stopped states", async () => {
	for (const [assistant, status, notification] of [
		...
		await stopped.mock.commands.get("goals")?.handler("resume", stopped.ctx);
...
test("manual compaction cancels stale continuation and sends one fresh continuation", async () => {
	...
	assert.match(freshContinuation, /pi-goals-continuation/);
...
test("stale goal tool calls are blocked after pause until a fresh non-goal prompt arrives", async () => {
	...
	paused.mock.events.get("input")?.[0]?.(
		{ source: "interactive", text: "/goals edit revised paused objective" },
		paused.ctx,
	);
...
test("findFinalAssistantMessage returns the last assistant with a known stop reason", () => {
	...
	assert.equal(validateObjective(""), "Usage: /goals <goal_to_complete>");
});
...
function assertHardenedGoalPrompt(prompt: string) {
	...
	assert.match(prompt, /goals_blocked.*true impasse.*three consecutive goal turns/is);
...
async function startGoalForTest(overrides: Record<string, unknown> = {}, command = "finish") {
	...
	mock.events.get("session_start")?.[0]?.({}, context.ctx);
	await mock.commands.get("goals")?.handler(command, context.ctx);
	return { mock, ...context };
}
...
function lastGoal(mock: ReturnType<typeof createMockPi>) {
	const entry = mock.entries.filter((entry) => entry.customType === "goals-state").at(-1);
	return ((entry?.data as { goals?: StoredGoal[] } | undefined)?.goals?.[0] ??
...
function findPersistedGoal(mock: ReturnType<typeof createMockPi>, status: string) {
	for (let index = mock.entries.length - 1; index >= 0; index--) {
		...
		if (entry?.customType !== "goals-state") continue;
		const stored = (entry.data as { goals?: StoredGoal[] } | undefined)?.goals?.find(
