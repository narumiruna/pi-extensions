...
- Symptom: pi-goal can stop on transient `rate_limit`, HTTP 429, or server errors before Pi retries. Cause: local usage-limit regexes run before Pi's retry classifier. Fix: reserve `usage_limited` for explicit subscription/quota/credit/billing exhaustion and delegate transient errors to `isRetryableAssistantError`.
