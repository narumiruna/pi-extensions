...
- Symptom: npm 12 `ci` reports missing `node-domexception@1.0.2` while npm 11 passes. Cause: the dependency comes through Pi's shrinkwrapped `fetch-blob` tree, whose root override is resolved differently. Fix: regenerate the lock with the pinned release npm version and keep CI/publish npm versions aligned.
- Symptom: alternate-Pi CI tests typecheck but fail runtime imports after npm 12 `install --no-save`. Cause: rewritten manifests conflict with the checked-in Pi dependency tree. Fix: use npm 11.16 for this compatibility-only install; do not disable the lockfile because that upgrades unrelated tooling.
