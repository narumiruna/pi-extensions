...
- Symptom: an unrelated `session_start` test can read or migrate the developer's real extension settings. Cause: lifecycle handlers load settings even when the assertion targets another warning. Fix: run every settings-loading lifecycle test with an isolated temporary `PI_CODING_AGENT_DIR` and a fresh module import.
